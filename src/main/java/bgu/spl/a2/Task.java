...
public abstract class Task<R> 
{
    ...
    protected final void spawn(Task<?>...task) 
    {
    	...
    	{
    		this.handleProc.getPool().myVec.get(handleProc.getId()).add((task[i]));
    		this.handleProc.getPool().vm.inc();
    	}
    }
    ...
    protected final void whenResolved(Collection<? extends Task<?>> tasks, Runnable callback) 
    {
    	...
    	AtomicInteger numOfRuning=new AtomicInteger(tasks.size());
        Iterator<?extends Task<?>> currTask=tasks.iterator();
        while(currTask.hasNext())
        {
        	(currTask.next()).getResult().whenResolved(() -> {
        		numOfRuning.decrementAndGet();
                if(numOfRuning.get()==0)
                {
                	callback.run();
